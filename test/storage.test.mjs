      ...
      const child = spawnSync(process.execPath, ["--input-type=module", "-e", `
        ...
        try {
          ...
          if (mode === 'cross-device') {
            ...
            const completed = planLegacyMigration(${JSON.stringify(project)});
            assert.equal(completed.completed.length, 1);
            assert.deepEqual(completed.blockers, []);
            fs.writeFileSync(recovered.target + '/state.json', '{"marker":"normal new work"}');
            assert.deepEqual(planLegacyMigration(${JSON.stringify(project)}).blockers, [], 'active runtime changes are not retired-evidence changes');
            fs.appendFileSync(saved + '/state.json', '\\nlate after completion');
            const late = planLegacyMigration(${JSON.stringify(project)});
            assert.equal(late.needed, false);
            assert.equal(late.completed[0].retired, saved);
            assert.deepEqual(late.completed[0].changes, [{ file: 'state.json', reason: 'changed' }]);
            assert.ok(late.blockers.length, 'late evidence must remain discoverable after the journal is gone');
            fs.writeFileSync(saved + '/state.json', globalState);
            fs.writeFileSync(recovered.target + '/state.json', globalState);
