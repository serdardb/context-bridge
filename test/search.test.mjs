...
test("branch search uses recorded history rather than the current checkout and needs no Git at read time", () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(JSON.parse(result.stdout).map((entry) => entry.file), [`${first}.md.consumed`]);
    assert.equal(searchProject(project, "migration").length, 3);
    ...
    assert.deepEqual(searchProject(project, "migration", { branch: "missing" }), []);
    ...
    assert.deepEqual(searchProject(project, "migration", { branch: "feature/first" }), []);
...
test("search finds evidence across checkpoint kinds without exposing physical paths", () => {
  ...
  const results = searchProject(project, "MIGRATION");
...
test("search CLI keeps option values out of the query and searches consumed inbound and outbound evidence", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.deepEqual(JSON.parse(result.stdout).map((entry) => entry.file), [names[2], names[1]]);
...
test("search refuses invalid date ranges and does not follow checkpoint symlinks", () => {
  ...
  assert.deepEqual(searchProject(project, "needle"), []);
