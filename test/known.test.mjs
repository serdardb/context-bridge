...
test("a chain carries what the target missed from EVERY agent, labelled by source", async () => {
  ...
  // Prefix capture and shape reads succeed; fail the probe's parse, actual
  // extraction or audit. Preflight must never authorize acknowledging loss.
  for (const fault of ["io", "malformed", "rewrite"]) for (const failAt of [3, 4, 5]) {
    saveState(project, s);
    fs.writeFileSync(claudeTranscript, sourceBytes);
    let reads = 0;
    fs.readSync = (file, ...args) => {
