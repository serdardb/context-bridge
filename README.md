...
- **Older registrations and filesystem support.** Automatic matching now includes directory birth time to distinguish recycled inodes. Older registrations are `unverified` and require an explicit `project adopt <id>` after you check ownership. Filesystems without a positive directory birth time currently cannot register or adopt projects; the command refuses rather than attach potentially unrelated context.
