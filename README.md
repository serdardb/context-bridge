...
- `/bridge <agent>` (a Claude plugin skill) and `$bridge <agent>` (a shared skill for Codex, Grok, Antigravity and OpenCode) are the same command with a target argument. The departing agent writes a reading of the work plus decisions and open questions, the bridge computes what the target is missing from every agent's native session files plus git, and delivers a bounded delta with Summary, Conversation, Decisions, Work and Next.
